...
class tree: ## tree class
    ...
    def calc_LBI(self, attr = 'lb', tau=0.0005, transform = lambda x:x):
        '''.
        ...
        '''
        self.postorder_node = sorted(tree.Objects, key=operator.attrgetter('height'), reverse = True)
        self.preorder_internal_node = sorted(tree.nodes, key = operator.attrgetter('height'), reverse = False)

        # traverse the tree in postorder (children first) to calculate msg to parents
        for node in self.postorder_node:
        ...
        for node in self.preorder_internal_node:
...
#node.__setattr__(attr, transform(tmp_LBI))
        setattr(self, attr, transform(tmp_LBI))
    ...
    while i < len(data): ## while there's characters left in the tree string - loop away
        ...
        if cerberus is not None:
            ...
            for vals in sets:
                ...
                #if'location' in tr:
                    #l1,l2 = val[1:-1].split(',')
                    #ll.cur_node.traits['location1'] = float(l1)
                    #ll.cur_node.traits['location2'] = float(l2)
