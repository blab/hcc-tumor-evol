...
available_analyses=['treeLength','RC','Sharp','tmrcas','transitions','subtrees','singlenode'] ## analysis names that are possible
        ...
        if treecount==0: ## At tree state 0 insert header into output file
            ...
            if lower==0 and upper==np.inf: ## only add a header if not doing a chunk
                ...
                if 'singlenode' in analyses:
                    ll.renameTips(tips)
                    tmrca=['C13|gamma|5.18|6.93','C19|gamma|9.02|4.56','C25|gamma|1.45|11.45','C31|gamma|7.78|8.17','C33|gamma|5.29|9.98','C35|gamma|2.81|11.67','C36|gamma|1.45|12.80','C3|gamma|3.48|5.69','C42|gamma|8.23|9.42','C46|gamma|3.03|13.14','C54|gamma|8.57|10.88','C57|gamma|4.50|13.71','C58|gamma|3.15|14.38','C5|gamma|6.53|2.64','C67|gamma|8.00|13.03','C68|gamma|6.65|13.82','C69|gamma|5.29|14.61','C6|gamma|5.18|5.24','C70|gamma|3.71|15.51','C79|gamma|6.65|15.29','C80|gamma|4.95|15.85','C8|gamma|1.23|8.06']
                ###########################################
...
#                                     outfile.write('\t%s'%(subtreeString))
            ...
            ###################################################
            if 'singlenode' in analyses:
                CA=ll.commonAncestor(tmrca,numName=False)
                #sys.stderr.write('\n%s'%(CA.traits.keys()))
                if len(CA.leaves)==len(tmrca): ## only interested if common ancestor contains the strict set of descendants
                    outfile.write('\t%s\t%s'%(CA.traits['location1'],CA.traits['location2']))
                else:
