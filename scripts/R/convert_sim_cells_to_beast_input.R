...
ggsave(filename = "figures/spatial_sim/cc_sampled_plot.png", cc_sampled_plot)
...
write.csv(CC_clustered_cells, file = "outputs/CC_clustered_cells.csv")
...
ggsave("figures/spatial_sim/clustered_plot.png",g)
...
all_muts <- define_mut_presence_absence(CC_sim_cells) %>% 
  as.data.frame %>% 
  add_column("index" = CC_sim_cells$index)

write.csv(all_muts, file = "outputs/all_muts.csv")
...
write.csv(final_sampled_muts, file = "outputs/CC_final_sampled_muts.csv")
...
all_muts <- length(isolate_muts(i = 1, mutations_vec = c(),
                         mutations_column = CC_sampled_cells$mutations))

print(paste0(ncol(all_muts), "mutations in population"))
print(paste0(ncol(final_sampled_muts), " sampled mutations"))
